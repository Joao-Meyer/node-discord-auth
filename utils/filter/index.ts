...
interface GetPageAndLimitOutput<ModelType extends string> {
  orderBy: object;
  distinct?: ModelType;
  where: object;
}
...
export const getGenericFilter = <QueryType extends string, ModelType extends string>({
  query,
  list
}: GetPageAndLimitInput<QueryType, ModelType>): GetPageAndLimitOutput<ModelType> => {
  const orderBy = {};
  const where: object[] = [];

  let distinct: ModelType | undefined;
  ...
  for (const item of list) {
    ...
    if (typeof query.distinct !== 'undefined' && String(item) === query.distinct)
